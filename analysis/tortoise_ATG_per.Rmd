---
title: "Peroxide Response Curves"
...
## Parameters:
```{r Parameters:}
treatments= c(0,10,100,500,1000,"N1000")
...
```{r Directory Hopping}
tort.per.time <- dir(
  path="../data/ATG/peroxide", 
  full.names = T,
  pattern = "(6|12|24|48)h"
) %>% 
  set_names(.,basename(.))

tort.per.time.plates <- tort.per.time %>% 
  lapply(
    .,
    function(x){
      x %>% 
...
tort.per.files <- tort.per.time.plates %>% 
  lapply(
    .,
    function(time){
      time %>% 
        ...
        function(plate, t, files){
          ...
          lapply(
              ...
              function(test, df, timepoint, p){
                df[[test]] %>%
                  reshape_96_wellplate(., value_name = "Value", Time=timepoint, Test = test, Drug="peroxide") %>%
...
tort.per.df %>% pull(cell_line) %>% unique


tort.per.full <- tort.per.df
#tort.per.full
```
```{r Write final results}
dir.create("../output/peroxide", showWarnings = F)
tort.per.full %>% write_tsv(.,"../output/peroxide/tort.ATG.peroxide.tsv")
```

## Plot
```{r, fig.align="center", message=F, warning=F, error=F}
species <- tort.per.full %>% pull(cell_line) %>% unique
species.passage <- tort.per.full %>% unite("sample", c("cell_line", "passage"), sep=", P", remove = F) %>% pull(sample) %>% unique
individual <- tort.per.df %>% pull(individual) %>% unique
individual.passage <- tort.per.df %>% unite("sample", c("individual", "passage"), sep=", P", remove = F) %>% pull(sample) %>% unique
...
fig6a <- tort.per.full %>%
  ...
  make_pretty_graph()+
  labs(
    title = "Tortoise ApoTox-Glo Peroxide Tx: 6 hours"
  )
fig6a + scale_x_log10()
```
