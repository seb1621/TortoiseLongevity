---
title: "Tortoise Hydrogen Peroxide"
author: "JV"
date: "4/26/2019"
...
## Data Loading:
```{r Directory Hopping}
tortoise.hpo.dir <- dir(
  path="../data/RTG/peroxide/", 
  full.names = T
  ) %>% 
  set_names(., basename(.) %>% gsub("3_RealTime-Glo_", "", .))
tortoise.hpo.files <- tortoise.hpo.dir %>% 
  lapply(
    .,
    function(x){
      dir(
...
tortoise.hpo.df <- suppressWarnings(
  tortoise.hpo.files %>% 
    names %>% 
    lapply(
      .,
          ...
          lapply(
            ...
            function(test, t, f){
              f[[t]][[test]] %>% 
                read_csv %>% 
                filter(`1`!="X") %>% 
                reshape_96_wellplate(., value_name = "Value", Time=t, Test = test, Drug = "Hydrogen Peroxide") %>% 
...
```{r Export Data}
tortoise.hpo.df2 %>% write_csv("../output/tortoise_perox_RTG.csv")
```


```{r Plot}
tortoise.rtg.hpo.plot <- tortoise.hpo.df2 %>% 
  ...
  labs(
    x="Time (h)",
    y="Normalized RLU",
    title = "Tortoises, Hydrogen Peroxide") 

tortoise.rtg.hpo.plot

```

```{r filter by hour}
tort.hpo.hour6 <- filter(tortoise.hpo.df2, Time == "4-29-2019_3-04-26_AM")
tort.hpo.hour18 <- filter(tortoise.hpo.df2, Time == "4-29-2019_2-51-13_PM")
```

```{r}
tortoise.rtg.hpo.box6 <- tort.hpo.hour6 %>% 
 unite("sample", c("cell_line", "passage"), sep=", P", remove = F) %>% 
  mutate(Treatment = paste(Treatment, " uM", sep="")) %>% 
  ggplot(
    aes(
  ...
  theme(
   ...
   labs(
      ...
      title= "Tortoise Peroxide Tx RealTime-Glo 6 hours"
      )
tortoise.rtg.hpo.box6
```

```{r}
tortoise.rtg.hpo.box18 <- tort.hpo.hour18 %>% 
 unite("sample", c("cell_line", "passage"), sep=", P", remove = F) %>% 
  mutate(Treatment = paste(Treatment, " uM", sep="")) %>% 
  ggplot(
    aes(
  ...
  theme(
   ...
   labs(
      x="Sample",
      y="Normalized RLU/RFU",
      title= "Tortoise Peroxide Tx RealTime-Glo 18 hours"
      )
tortoise.rtg.hpo.box18
```
