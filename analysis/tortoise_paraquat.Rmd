...
## Parameters:
```{r Parameters:}
treatments.unique = c(0,50,500,2500)
...
## Data Loading:
```{r Directory Hopping}
tortoise.pqt.dir <- dir(
  path="../data/RTG/paraquat/", 
  full.names = T
  ) %>% 
  set_names(., basename(.) %>% gsub("2_RealTime-Glo_", "", .))
tortoise.pqt.files <- tortoise.pqt.dir %>% 
  lapply(
    .,
    function(x){
      dir(
...
tortoise.pqt.df <- suppressWarnings(
  tortoise.pqt.files %>% 
    names %>% 
    lapply(
      .,
          ...
          lapply(
            ...
            function(test, t, f){
              f[[t]][[test]] %>% 
                read_csv %>% 
                filter(`1`!="X") %>% 
                reshape_96_wellplate(., value_name = "Value", Time=t, Test = test, Drug = "Paraquat") %>% 
...
```{r Export Data}
tortoise.pqt.df2 %>% write_csv("../output/tortoise_paraq_RTG.csv")
```

```{r Plot}
tortoise.rtg.pqt.plot <- tortoise.pqt.df2 %>% 
  ...
  labs(
    x="Time (h)",
    y="Normalized RLU",
    title = "Tortoises, Paraquat"
  )
tortoise.rtg.pqt.plot

```

```{r filter by hour}
tort.pqt.hour24 <- filter(tortoise.pqt.df2, Time == "4-29-2019_8-49-32_PM")
tort.pqt.hour48 <- filter(tortoise.pqt.df2, Time == "4-30-2019_8-44-17_PM")
```


```{r}
tortoise.rtg.pqt.box24 <- tort.pqt.hour24 %>% 
 unite("sample", c("cell_line", "passage"), sep=", P", remove = F) %>% 
  mutate(Treatment = paste(Treatment, " uM", sep="")) %>% 
  ggplot(
    aes(
  ...
  theme(
   ...
   labs(
      ...
      title= "Tortoise Paraquat Tx RealTime-Glo 24 hours"
      )
tortoise.rtg.pqt.box24
```

```{r}
tortoise.rtg.pqt.box48 <- tort.pqt.hour48 %>% 
 unite("sample", c("cell_line", "passage"), sep=", P", remove = F) %>% 
  mutate(Treatment = paste(Treatment, " uM", sep="")) %>% 
  ggplot(
    aes(
  ...
  theme(
   ...
   labs(
      x="Sample",
      y="Normalized RLU/RFU",
      title= "Tortoise Paraquat Tx RealTime-Glo 48 hours"
      )
tortoise.rtg.pqt.box48
```
