...
```{r Directory Hopping}
turtle.pqt.dir <- dir(
  path="../data/Tort_paraquat/", 
  full.names = T
  ) %>% 
  set_names(., basename(.) %>% gsub("2_RealTime-Glo_", "", .))
turtle.pqt.files <- turtle.pqt.dir %>% 
  lapply(
    .,
    function(x){
      dir(
...
turtle.pqt.df <- suppressWarnings(
  turtle.pqt.files %>% 
    names %>% 
    lapply(
      .,
...
```{r}
turtle.rtg.pqt.plot <- turtle.pqt.df2 %>% 
