---
title: "Turtle Hydrogen Peroxide"
author: "JV"
date: "4/26/2019"
...
```{r Directory Hopping}
turtle.hpo.dir <- dir(
  path="../data/Tort_peroxide/", 
  full.names = T
  ) %>% 
  set_names(., basename(.) %>% gsub("3_RealTime-Glo_", "", .))
turtle.hpo.files <- turtle.hpo.dir %>% 
  lapply(
    .,
    function(x){
      dir(
...
turtle.hpo.df <- suppressWarnings(
  turtle.hpo.files %>% 
    names %>% 
    lapply(
      .,
...
```{r}
turtle.rtg.hpo.plot <- turtle.hpo.df2 %>% 
  ...
  labs(
    x="Time (h)",
    y="Normalized RLU (log)",
    title = "Turtles, Hydrogen Peroxide"
  ) + 
  scale_y_log10()
turtle.rtg.hpo.plot
